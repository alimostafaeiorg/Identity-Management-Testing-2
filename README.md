...
### WSTG-IDNT-04
# تست برای شمارش حساب‌های کاربری و حساب‌های قابل حدس 

### هدف تست:
####بررسی اینکه آیا سیستم به گونه‌ای طراحی شده که با تحلیل پیام‌های خطا یا رفتار آن در فرآیندهای ثبت‌نام، ورود یا بازیابی رمز عبور، امکان شمارش کاربران (User Enumeration) یا اجرای حملات Brute Force فراهم باشد یا خیر.
مراحل تست

 ##### بررسی پیام‌های خطا:
##### در فرم ورود یا ثبت‌نام، نام کاربری یا ایمیلی که در سیستم وجود ندارد را وارد کنید و بررسی کنید که آیا پیام خطای نمایش داده‌شده نشان‌دهنده موجود نبودن حساب کاربری است.
##### برای مثال:
            "User does not exist." (اطلاعات مستقیم)
            "Incorrect password." در مقایسه با "User not found."

##### تحلیل رفتار سیستم:
##### در بخش‌های مختلف، مانند ثبت‌نام، فراموشی رمز عبور یا ورود، رفتار سیستم را برای ورودی‌های صحیح و ناصحیح مقایسه کنید.
##### آیا لینک‌های تغییر مسیر یا URLها تفاوتی در پردازش ورودی صحیح و ناصحیح دارند؟
##### آیا می‌توان با تحلیل زمان پاسخ‌دهی متوجه شد که کاربر موردنظر وجود دارد یا خیر؟
##### مثال: پاسخ سریع‌تر در صورت عدم وجود کاربر در مقایسه با حالتی که رمز عبور نادرست وارد شده است.

 #### بررسی تلاش‌های brute force:
##### آیا تعداد تلاش‌های ورود یا ثبت‌نام محدود شده است؟
##### آیا از کپچا یا دیگر مکانیزم‌های امنیتی استفاده می‌شود؟

### راهکارهای جلوگیری

#### یکنواخت‌سازی پیام‌های خطا:
#####پیام‌های خطا باید عمومی و یکسان باشند. به‌عنوان‌مثال:
###### Credentials submitted are not valid
#####این پیام هیچ اطلاعات خاصی درباره وجود یا عدم وجود کاربر در سیستم ارائه نمی‌دهد.

 #### حذف حساب‌های پیش‌فرض:
##### حساب‌های پیش‌فرض یا آزمایشی که ممکن است پس از انتشار سیستم باقی مانده باشند، باید حذف شوند.

#### محدودیت تعداد تلاش:
#####برای تلاش‌های ناموفق ورود، یک مکانیزم محدودکننده ایجاد کنید:
##### قفل کردن حساب پس از تعداد مشخصی تلاش ناموفق.
##### اعمال تأخیر زمانی در پاسخ‌دهی پس از چند تلاش ناموفق.

 #### استفاده از کپچا:
#####اضافه کردن کپچا برای جلوگیری از حملات خودکار مانند brute force یا bot attacks.

#### عدم افشای زمان پاسخ:
##### سیستم نباید زمان پاسخ‌دهی متفاوتی برای ورودی‌های درست و غلط ارائه دهد که می‌تواند نشانه‌ای از وجود کاربر باشد.

#### بررسی امنیت URLها:
 ##### اطمینان حاصل کنید که URLهای تغییر مسیر یا پارامترهای موجود در آنها اطلاعات اضافی درباره صحت نام کاربری یا ایمیل ارائه نمی‌دهند.
...
#### بررسی کنید آیا ساختار نام‌های کاربری برنامه به گونه‌ای است که سیستم را نسبت به شمارش کاربران (User Enumeration) آسیب‌پذیر می‌کند.
#### ارزیابی کنید آیا پیام‌های خطای برنامه اطلاعاتی در اختیار مهاجم قرار می‌دهند که امکان شمارش کاربران را فراهم کند.

### روش آزمایش:

#### شناخت ساختار نام‌های کاربری:
#### تحلیل کنید آیا محدودیت یا الگوی خاصی برای نام‌های کاربری وجود دارد (مانند طول، حروف مجاز، استفاده از اعداد، یا استفاده از کاراکترهای خاص).
####  بررسی کنید که آیا سیاست تعریف‌شده به گونه‌ای است که ایجاد نام‌های کاربری قابل پیش‌بینی یا حدس زدن را آسان می‌کند.

#### ارزیابی پاسخ برنامه:
####  نام‌های کاربری معتبر و نامعتبر را وارد کنید و پیام‌های خطای برنامه را بررسی کنید.
####  ببینید آیا برنامه تفاوت قابل‌تشخیصی بین پیام‌های خطای مربوط به نام کاربری معتبر و نامعتبر نشان می‌دهد.

### تحلیل تفاوت پاسخ‌ها:
#### با بررسی تفاوت‌های زمانی یا تغییرات در پیام‌ها و مسیرهای درخواست، نام‌های کاربری معتبر را شناسایی کنید.

### استفاده از دیکشنری:
 #### از لیستی از نام‌های کاربری رایج یا حدسی برای تست صحت نام‌های کاربری موجود در سیستم استفاده کنید.

### راهکارها:

#### یکسان‌سازی پیام‌های خطا:
#### پیام‌های خطای سیستم باید عمومی و یکسان باشند و هیچ اطلاعات خاصی درباره اعتبار یا وجود نام کاربری ارائه ندهند. به‌عنوان‌مثال:
        "Invalid login credentials"
 ####  به جای پیام‌هایی مانند "Username does not exist" یا "Password is incorrect."

#### اعمال محدودیت روی نام‌های کاربری:
##### از کاربران بخواهید نام‌های کاربری پیچیده و غیرقابل پیش‌بینی انتخاب کنند.
##### محدودیت‌هایی مانند حداقل طول، استفاده از حروف کوچک و بزرگ، و ممنوعیت استفاده از الگوهای ساده را اعمال کنید.

#### حفاظت در برابر تلاش‌های بیش‌ازحد:
 ##### با اعمال محدودیت تعداد تلاش‌های ناموفق یا استفاده از کپچا، از سوءاستفاده برای شمارش کاربران جلوگیری کنید.

#### حذف اطلاعات حساس:
#####  اطمینان حاصل کنید که درخواست‌های API یا پاسخ‌ها شامل اطلاعات خاص درباره صحت یا عدم صحت نام کاربری نیستند.
